...
const rewardIcon = (type, name, compact = false) => {
  ...
  return `
    <span class="reward-icon${compact ? ' compact' : ''}${media?.url ? ' has-media' : ''}">
      ${rewardFallback(type, compact)}
      ${media?.url ? `<img src="${esc(media.url)}" alt="" loading="lazy" referrerpolicy="no-referrer" data-reward-image>` : ''}
    </span>`;
};
...
const vaultFallback = (entry, detail = false) => `
  <span class="vault-fallback${detail ? ' detail' : ''}" aria-hidden="true">
    ...
    <strong>${esc(initials(entry.name))}</strong>
  </span>`;
...
const cover = (entry, detail = false) => {
  ...
  return `
    <span class="pack-icon${detail ? ' detail' : ''}${media?.url ? ' has-media' : ''}">
      ...
      ${media?.url ? `<img src="${esc(media.url)}" alt="${esc(entry.name)} icon" loading="lazy" referrerpolicy="no-referrer" data-cover-image>` : ''}
    </span>`;
};
...
const card = (entry) => {
  ...
  const previews = rewards.slice(0, 4)
    .map(({ type, name }) => `<span title="${esc(clean(name))}">${rewardIcon(type, name, true)}</span>`)
    .join('');
  ...
  return `
    <button class="pack-card" type="button" data-open="${esc(entry.slug)}" aria-label="Open ${esc(entry.name)} details">
      ${cover(entry)}
      <span class="pack-card-body">
        <span class="pack-card-meta">
          <time datetime="${esc(entry.releaseDate)}">${esc(entry.releaseLabel)}</time>
          ${entry.hasAccountUnlock ? '<span class="account-dot" title="Includes account unlocks">Account</span>' : ''}
        </span>
        <strong class="pack-card-title">${esc(entry.name)}</strong>
        <span class="pack-card-types">${types || '<span class="type-chip muted">Pack</span>'}</span>
...
const detailSection = (title, type, items) => {
  ...
  return `
    <section class="detail-section">
      <header><h3>${esc(title)}</h3><span>${items.length}</span></header>
          ...
          return `
            <article class="reward-card">
              ...
              <span>
                ...
                <small>${esc(media?.provider || typeLabels[type] || 'Reward')}</small>
              </span>
            </article>`;
        }).join('')}
      </div>
...
const openDetails = (slug, setHash = true) => {
  ...
  const coverMedia = resolveCoverMedia(entry);
  const source = coverMedia?.sourceUrl || entry.sourceUrl || entry.imageDiscovery?.pageUrl || mediaSources.nwhub.url;
  const rewards = allRewards(entry);
  ...
  el.content.innerHTML = `
    <header class="detail-header">
      ${cover(entry, true)}
      <div>
        <p class="eyebrow">${esc(entry.releaseLabel)} · ${esc(entry.platform)}</p>
    ...
    <div class="detail-content">
      ...
      ${detailSection('Special packs', 'race', entry.rewards?.races || [])}

      <div class="detail-actions">
        <button class="button primary" type="button" data-copy="${esc(entry.slug)}">Copy link</button>
        <a class="button" href="${esc(source)}" target="_blank" rel="noreferrer">Artwork / source</a>
        <a class="button" href="${esc(mediaSources.nwhub.url)}" target="_blank" rel="noreferrer">NW Hub</a>
      </div>
    </div>`;
...
const showToast = (message) => {
  ...
  showToast.timer = setTimeout(() => { el.toast.hidden = true; }, 2000);
};
...
const loadOptionalMedia = async () => {
  try {
    ...
    console.warn('Optional media services are unavailable. Local fallbacks will remain in use.', error);
  }
};
