...
const runtimeMedia = await readFile(new URL('../data/runtime-media.js', import.meta.url), 'utf8');
...
test('viewer-facing shell has its own identity and no external catalog branding', () => {
  ...
  assert.doesNotMatch(artwork, /NW Hub|nw-hub\.com/i);
...
test('runtime keeps local fallbacks when artwork is unavailable', () => {
  ...
  assert.match(runtimeMedia, /["']?lockbox["']?\s*:/);
});
